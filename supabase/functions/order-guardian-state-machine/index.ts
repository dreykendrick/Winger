// Winger Backend V2 - Order Guardian State Machine Edge Function
...
interface StateTransitionPayload {
  order_id: string;
  target_status: 'SHIPPED' | 'DELIVERED' | 'DISPUTED';
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const body: StateTransitionPayload = await req.json();
    ...
    // Execute state machine transition procedure
    const { data: transitionResult, error: rpcError } = await supabase.rpc('order_guardian.fn_transition_order_status', {
    ...
    if (rpcError) {
      return buildErrorResponse(`State transition failed: ${rpcError.message}`, 'STATE_TRANSITION_FAILED', 400);
    }

    return buildSuccessResponse(transitionResult, 'Order status transitioned successfully', 'ORDER_STATUS_UPDATED', 200);
  } catch (err) {
